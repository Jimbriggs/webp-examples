...
/**
 * Re-assigns project moderator
 * @author Steve Powell
 */
public class ModeratorAssign extends PumsBaseAction {
    
    /** Creates a new instance of SPAddProject */
    public ModeratorAssign() {
    }
    ...
    public ActionForward doPumsExecute(StrutsUtils su) throws IOException, ServletException, JimFatalException {
        if(isTokenValid(su.getRequest())) {
            ...
            StaffBean moderator = (StaffBean)su.getForm();
            
            try {
                Connection conn = su.getConn();
                su.beginTransaction();
                ProjectService projectservice = new ProjectService(conn);
                String assigned = projectservice.getCapacityAlreadyAssigned(moderator.getID(), project.getMarkers());
                if(assigned==null) {
                    project = projectservice.updateMarker(project, moderator.getID(), "Moderator");
                } else {
